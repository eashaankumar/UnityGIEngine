...
{
    // Thanks to Seb Lague (https://www.youtube.com/watch?v=Qz0KTGYJtUk 12:51)
    float theta = 2 * K_PI * RandomFloat01(seed);
    float rho = sqrt(-2 * log(RandomFloat01(seed)));
    return rho * cos(theta);
}

float3 RandomDirectionNormalDistribution(inout uint state)
{
...
float3 SampleHemisphereNormalDistribution(float3 n, inout uint state)
{
    float3 randomDir = RandomDirectionNormalDistribution(state);
    return randomDir * sign(dot(n, randomDir));
}
