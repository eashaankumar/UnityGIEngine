Shader "Unlit/RayTracing/Diffuse"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            {
                //float3 diffuseRayDir = normalize(faceNormal + RandomDirectionNormalDistribution(rngState));

                //float3 diffuseRayDir = SampleHemisphere(v.normal, payload.rngState);
                
                float3 diffuseRayDir = SampleHemisphereNormalDistribution(faceNormal, rngState);
